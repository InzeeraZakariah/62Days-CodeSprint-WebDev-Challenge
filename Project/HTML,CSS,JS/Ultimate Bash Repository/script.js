// Primary framework configuration pairing UI layouts to curriculum folders (Bash-focused)
const topicsData = {
    "Bash Fundamentals": {
        mdFile: "Bash Fundamentals Overview.md",
        subtopics: [
            { name: "Basic Syntax", path: "Bash_Fundamentals/Basic_Syntax" },
            { name: "Variables", path: "Bash_Fundamentals/Variables" },
            { name: "Permissions", path: "Bash_Fundamentals/Permissions" }
        ]
    },
    "Scripting Constructs": {
        mdFile: "Scripting Constructs Overview.md",
        subtopics: [
            { name: "Conditional Logic", path: "Scripting_Constructs/Conditional_Logic" },
            { name: "Loops", path: "Scripting_Constructs/Loops" },
            { name: "Functions", path: "Scripting_Constructs/Functions" }
        ]
    },
    "Practical Examples": {
        mdFile: "Practical Examples Overview.md",
        subtopics: [
            { name: "File Management", path: "Practical_Examples/File_Management" },
            { name: "System Information", path: "Practical_Examples/System_Information" }
        ]
    }
};
...
function updateBrowserTabTitle(viewName) {
    document.title = `${viewName} | 💻 Ultimate Bash Repository`;
}
...
async function checkUrlHashRoute() {
    ...
    if (!currentHash) {
        document.title = "Ultimate Bash Repository";
        return; 
    }
...
function cleanDisplayName(rawName) {
    return rawName.replace(/\.(sh|md|png|jpg|jpeg)$/i, '').replace(/^\d+_/g, '');
}
...
async function resolveAndBuildContent(folderPath, targetContainer, currentSidebarUL, preferredOrder = []) {
    ...
    for (const item of immediateContents) {
        ...
        if (item.type === "blob") { 
            ...
            const isCodeFile = lowerName.endsWith('.html') || lowerName.endsWith('.css') || lowerName.endsWith('.js') || lowerName.endsWith('.php') || lowerName.endsWith('.sql') || lowerName.endsWith('.sh');
            ...
            if (shouldFlatten) {
                ...
                const isSingleCodeFile = singleLowerName.endsWith('.html') || singleLowerName.endsWith('.css') || singleLowerName.endsWith('.js') || singleLowerName.endsWith('.php') || singleLowerName.endsWith('.sql') || lowerName.endsWith('.sh');
